...
- [~] Shared UI primitives — components/ui now has Button / Input / Textarea / Badge / Modal / EmptyState, each encoding the dominant existing classes so migration is visually identical. The People module is fully migrated (proof). Remaining: roll the rest of the modules (candidates, jobs, pilot-requirements, calendar, compliments, settings, etc.) onto the primitives, module by module. (Surfaced by the Jun 30 design audit; foundations shipped Jun 30.)
