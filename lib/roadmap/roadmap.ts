...
- [x] Central Travel page (Jun 23) — a Travel hub under People (/travel) listing every trip across new hires AND candidates, not just orientation: filter by status / purpose / hire-vs-candidate / search, top-line totals (trips, needs-booking, booked, total spend), and each row crisscrosses to that person's record (where trips are added/edited). Travel is its own thing now, with the per-person panels + Orientation chips linking around it.
