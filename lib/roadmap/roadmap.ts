...
- [ ] Shared UI primitives — extract Button / Input / Modal / Tab / Badge / EmptyState components (today these are ~150+ ad-hoc inline-Tailwind variations, so hover/focus/disabled states drift). The Card pattern is already consistent; do the rest. Cuts future inconsistency at the source. (Surfaced by the Jun 30 design audit.)
- [ ] Accessibility pass (WCAG 2.1 A/AA) — add visible focus rings to interactive elements (only ~65 focus: styles today), aria-labels on icon-only buttons, an icon/shape alongside color-only status pills, and lift low-contrast brand-grey body text to meet 4.5:1. (Surfaced by the Jun 30 design audit.)
- [ ] Dark-mode token + gaps — replace the hardcoded dark card color (dark:bg-[#10243a], ~100+ literals) with a single theme token, and add dark: variants to the few remaining status banners (emerald/amber success/warning boxes) that only have light styles. (Surfaced by the Jun 30 design audit.)
