...
- [x] Self-service hire import (Jun 23) — "Import hires" button on Pre-onboarding: paste rows straight from the roster spreadsheet (or upload a CSV), it maps the columns (Name, Offer Sent/Signed, Start, Orientation, Phone, SS Email, Personal Email, Position, Department) onto each profile, previews who will be added, and skips anyone already in the system (by name/email) so re-uploading the whole sheet only adds the new people. Each imported hire gets the standard checklist.
...
- [x] Pre-onboarding dashboard drill-downs (Jun 23) — the headline metric cards (Starting in 7 days, Hires with missing items, Needs attention) are now clickable and expand an inline list of exactly those people, each linking to their record. More dashboard metrics (orientation timeliness, etc.) are being brainstormed.
