...
- [x] Document checklist + type filter (Jun 14) — candidate profile sidebar shows a real document checklist driven by the type tags: each expected document is green (on file) or red (required + missing) / muted (optional + missing), with an X/Y summary; the Documents tab gained type-filter chips to narrow files by tag. Presence-based now; currency/expiry coloring needs expiry-date capture (Medical/Passport) — follow-up
