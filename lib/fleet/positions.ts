...
export function resolveFleetPosition(rawTitle: string | null | undefined): FleetPosition | null {
  ...
  const aircraftMatchers: Array<{ test: RegExp; aircraft: string }> = [
    ...
    // Challenger 300 and 350 share the CL-30 rating and we only operate the 350,
    // so a post naming either resolves to the 350 position.
    { test: /\bchallenger ?3[05]0\b|\bcl-?3[05]0\b/, aircraft: "Challenger 350" },
    { test: /\bpraetor ?600\b|\bemb-?550\b/, aircraft: "Praetor 600" },
