...
export const SCAN_EXCLUSION_REASONS: Array<{ key: ScanExclusionReason; label: string; hint: string }> = [
  ...
  {
    key: "NOT_ELIGIBLE",
    label: "Not eligible to hire",
    hint: "Work authorization / visa / a legal bar — unlike a culture call this does not expire on its own"
  },
  { key: "OTHER", label: "Other", hint: "Another reason — add a note" }
];

/**
 * Reasons that are a standing bar rather than a judgement call about one moment.
 * Used to word the re-apply flag more firmly when someone returns.
 */
export const PERMANENT_SCAN_EXCLUSION_REASONS: ScanExclusionReason[] = ["NOT_ELIGIBLE", "TEST"];
...
const REASON_KEYS = new Set<string>(SCAN_EXCLUSION_REASONS.map((reason) => reason.key));
