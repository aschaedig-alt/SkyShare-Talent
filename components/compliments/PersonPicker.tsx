...
export function PersonPicker({
  ...
  if (selected) {
    return (
      <div>
        <p className="mb-2 block text-sm font-medium text-brand-lea">{label}</p>
        <div className="flex items-center gap-2.5 rounded-element bg-brand-cloudDancer/60 px-3 py-2">
          <Avatar name={selected.name} initials={selected.initials} size="sm" />
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-brand-lea">{selected.name}</p>
          ...
          <button
            ...
            className="rounded p-1 text-brand-grey transition hover:bg-white hover:text-brand-lea"
  ...
  return (
    <div>
      <label className="mb-2 block text-sm font-medium text-brand-lea">{label}</label>
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-element border-[0.5px] border-brand-lea/20 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-value-innovation focus:ring-2 focus:ring-value-innovation/30"
        />
        {open && matches.length > 0 ? (
          <ul className="absolute z-10 mt-1 max-h-64 w-full overflow-auto rounded-element border-[0.5px] border-brand-lea/15 bg-white py-1 shadow-panel">
            {matches.map((p) => (
              <li key={p.id}>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm text-brand-lea">{p.name}</p>
                    <p className="truncate text-xs text-brand-grey">
