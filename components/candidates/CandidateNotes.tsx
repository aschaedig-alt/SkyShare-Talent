    ...
    <section className="rounded-xl bg-white p-4 shadow-panel ring-1 ring-brand-lea/10">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
        <label className="flex-1">
          <span className="mb-1 block text-[11px] font-bold uppercase tracking-wide text-brand-grey">Add a note</span>
          <textarea
            value={draft}
            onChange={(e) => setDraft(e.target.value)}
            rows={2}
            placeholder="Record a note about this candidate…"
            className="w-full rounded-lg border border-brand-lea/20 px-3 py-2 text-sm outline-none transition focus:border-brand-gold"
          />
        </label>
        <button
          type="button"
      ...
      <div className="mt-4 space-y-2">
        {notes.length === 0 ? (
          <p className="py-6 text-center text-sm text-brand-grey">No notes yet. Add the first one above.</p>
        ) : (
          notes.map((note) => (
            <div key={note.id} className="group rounded-lg border border-brand-lea/10 bg-brand-cloudDancer/45 p-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <button
                  ...
                  className="shrink-0 rounded p-1 text-brand-grey/0 transition group-hover:text-brand-grey hover:!text-red-600"
                >
