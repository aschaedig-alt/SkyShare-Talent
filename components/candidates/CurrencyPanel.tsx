  ...
  return (
    <div className="rounded bg-white p-4 shadow-panel ring-1 ring-brand-lea/10 dark:bg-brand-panel dark:ring-white/10">
      <div className="flex items-center justify-between">
        ...
        {dated.length > 0 && (
          <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${anyAlert ? "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300" : "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300"}`}>
      ...
      {dated.length > 0 && (
        <ul className="mt-3 space-y-1.5">
          {dated.map((d, i) => {
            const tone = d.days < 0 ? "text-red-600 dark:text-red-300" : d.days <= 30 ? "text-amber-700 dark:text-amber-300" : "text-emerald-700 dark:text-emerald-300";
