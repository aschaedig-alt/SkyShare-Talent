      ...
      <div className="mt-3">
        ...
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          ...
          {pros.map((p) => (
            <span key={p} className="inline-flex items-center gap-1 rounded border border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/15 px-2.5 py-1 text-[11px] font-semibold text-emerald-800 dark:text-emerald-300">
              {p}
              <button type="button" onClick={() => removePro(p)} aria-label={`Remove ${p}`} className="text-emerald-600/70 transition hover:text-emerald-900 dark:text-emerald-400/70 dark:hover:text-emerald-200">
                <X className="h-3 w-3" />
              </button>
            </span>
          ))}
        ...
        <div className="mt-2 flex gap-1.5">
          <input
            ...
            className="min-w-0 flex-1 rounded border border-brand-lea/20 px-2.5 py-1.5 text-sm outline-none transition focus:border-brand-gold dark:border-white/10 dark:text-slate-100"
          />
          <button type="button" onClick={addPro} disabled={!newPro.trim()} aria-label="Add strength" className="rounded border border-emerald-200 px-2 py-1.5 text-emerald-700 transition hover:bg-emerald-50 disabled:opacity-50 dark:border-emerald-500/30 dark:text-emerald-400 dark:hover:bg-emerald-500/15">
      ...
      <div className="mt-4">
        ...
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          ...
          {cons.map((c) => (
            <span key={c} className="inline-flex items-center gap-1 rounded border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/15 px-2.5 py-1 text-[11px] font-semibold text-amber-800 dark:text-amber-300">
              {c}
              <button type="button" onClick={() => removeCon(c)} aria-label={`Remove ${c}`} className="text-amber-600/70 transition hover:text-amber-900 dark:text-amber-400/70 dark:hover:text-amber-200">
                <X className="h-3 w-3" />
              </button>
            </span>
          ))}
        ...
        <div className="mt-2 flex gap-1.5">
          <input
            ...
            className="min-w-0 flex-1 rounded border border-brand-lea/20 px-2.5 py-1.5 text-sm outline-none transition focus:border-brand-gold dark:border-white/10 dark:text-slate-100"
          />
          <button type="button" onClick={addCon} disabled={!newCon.trim()} aria-label="Add concern" className="rounded border border-amber-200 px-2 py-1.5 text-amber-700 transition hover:bg-amber-50 disabled:opacity-50 dark:border-amber-500/30 dark:text-amber-400 dark:hover:bg-amber-500/15">
            <Plus className="h-4 w-4" />
          </button>
        </div>
      </div>
