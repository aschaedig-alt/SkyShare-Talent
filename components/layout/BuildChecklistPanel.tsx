...
export function BuildChecklistPanel() {
  return (
    <aside className="rounded bg-white p-4 shadow-panel ring-1 ring-brand-lea/10">
      ...
      <div className="mt-4">
        <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand-grey">
          Completed foundation
        </div>
        <div className="mt-2 space-y-2">
          {completedBuildItems.map((item) => (
            <div key={item} className="flex gap-2 rounded border border-brand-lea/10 bg-brand-cloudDancer/45 p-2">
              <span className="mt-0.5 h-3 w-3 rounded-full bg-emerald-500" />
              <span className="text-xs font-semibold leading-5 text-brand-lea">{item}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 space-y-2">
          {upcomingBuildItems.map((item, index) => (
            <div key={item} className="flex gap-2 rounded border border-brand-lea/10 bg-white p-2">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-gold/20 text-[11px] font-bold text-brand-lea">
                {index + 1}
              </span>
