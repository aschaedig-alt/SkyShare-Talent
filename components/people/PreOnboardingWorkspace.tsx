      ...
      <div className="border-b border-brand-lea/10">
        <nav className="flex flex-wrap gap-6">
          {tabs.map((t) => (
            ...
            >
              {t.label}
              {t.badge !== undefined ? <span className="ml-1.5 text-brand-grey">· {t.badge}</span> : null}
            </Link>
          ))}
        </nav>
      </div>
      ...
      {adding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded bg-white p-5 shadow-2xl">
            ...
            <p className="mt-1 text-sm text-brand-grey">Creates an active hire with the standard checklist.</p>
