      ...
      <main className="space-y-4">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
          {statLabels.map(([key, label, href]) => (
            <Link key={key} href={href} className="rounded bg-white p-3 shadow-panel ring-1 ring-brand-lea/10 transition hover:ring-brand-sweet hover:shadow-glow">
          ...
          <section className="rounded bg-white p-4 shadow-panel ring-1 ring-brand-lea/10">
            ...
            <div className="mt-3 space-y-2">
              {data.attentionItems.map((item) => (
                <Link key={item.label} href={item.href} className="block rounded border border-brand-lea/10 bg-brand-cloudDancer/45 p-3 transition hover:border-brand-sweet hover:bg-brand-sweet/18 hover:shadow-glow">
          ...
          <section className="rounded bg-white p-4 shadow-panel ring-1 ring-brand-lea/10">
            ...
            <div className="mt-3 space-y-2">
              {data.recentCandidates.map((candidate) => (
                <Link key={candidate.id} href={`/candidates/${candidate.id}`} className="block rounded border border-brand-lea/10 bg-brand-cloudDancer/45 p-3 transition hover:border-brand-sweet hover:bg-brand-sweet/18 hover:shadow-glow">
        ...
        <section className="rounded bg-white p-4 shadow-panel ring-1 ring-brand-lea/10">
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {data.recentJobs.map((job) => (
              <Link key={job.id} href={`/recruiting-jobs?id=${job.id}`} className="rounded border border-brand-lea/10 bg-brand-cloudDancer/45 p-3 transition hover:border-brand-sweet hover:bg-brand-sweet/18 hover:shadow-glow">
