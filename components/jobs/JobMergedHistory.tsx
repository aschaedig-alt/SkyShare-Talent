...
export function JobMergedHistory() {
  ...
  return (
    <section className="mt-4 rounded border border-brand-lea/10 bg-white">
      <button
        onClick={toggleOpen}
        className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition hover:bg-brand-cloudDancer/20 hover:shadow-glow"
      >
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-brand-gold">Merged job history</p>
          <h3 className="text-sm font-semibold text-brand-lea">
            Previously merged jobs{items ? ` (${items.length})` : ""}
          </h3>
        </div>
        <ChevronDown className={`h-5 w-5 text-brand-grey transition-transform ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="border-t border-brand-lea/10 p-3">
          {message && (
            <div
          ...
          {loading && (
            <div className="flex justify-center py-6">
              <Loader className="h-5 w-5 animate-spin text-brand-lea" />
            </div>
          )}
          ...
          {!loading && !error && items && items.length === 0 && (
            <p className="px-2 py-4 text-center text-sm text-brand-grey">No jobs have been merged yet.</p>
          )}
          ...
          {!loading && items && items.length > 0 && (
            <div className="space-y-2">
              {items.map((job) => (
                <div
                  key={job.id}
                  className="flex flex-wrap items-center justify-between gap-3 rounded border border-brand-lea/10 bg-brand-cloudDancer/20 p-3"
                >
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-brand-lea">{job.title}</div>
                    <div className="mt-0.5 text-xs text-brand-grey">
                      ...
                      {job.mergedInto ? (
                        <Link
                          ...
                          target="_blank"
                          className="font-medium text-brand-lea underline hover:text-brand-gold transition hover:shadow-glow"
                        >
                    ...
                    <div className="mt-1 text-[11px] text-brand-grey">
                      {job.reversible
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 rounded border border-brand-lea/20 px-3 py-2 text-sm font-semibold text-brand-lea transition hover:bg-brand-sweet/20 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {unmergingId === job.id ? (
