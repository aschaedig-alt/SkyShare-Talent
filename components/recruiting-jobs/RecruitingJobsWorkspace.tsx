...
// Right-hand detail region: header on top, candidates | requirements side by side
// (candidates first, per the Layout Lab arrangement), then the source record full width.
function JobDetailRegion({ job }: { job: RecruitingJobDetail | null }) {
  if (!job) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 2xl:grid-cols-2">
        <LinkedCandidates job={job} />
        <LinkedRequirements job={job} />
      </div>
      <SourceRecord job={job} />
    </div>
  );
}
...
export function RecruitingJobsWorkspace({ data, query }: RecruitingJobsWorkspaceProps) {
  ...
  return (
    <div className="space-y-4 px-5 py-5 lg:px-8">
      {/* Top band: role operations header (left) + statistics (right) — Layout Lab arrangement */}
      <div className="grid gap-4 xl:[grid-template-columns:15fr_21fr]">
        <section className="rounded bg-white p-5 shadow-panel ring-1 ring-brand-lea/10">
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-brand-gold">
            Role operations
          </p>
      ...
      {/* Main band: pilot list | support list | detail region — three columns (Layout Lab) */}
      <div className="grid gap-4 xl:[grid-template-columns:10fr_10fr_16fr]">
        ...
        <JobDetailRegion job={data.selectedJob} />
      </div>
    </div>
  );
}
