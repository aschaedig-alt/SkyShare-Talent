...
const FIELD = "w-full rounded border border-brand-lea/20 bg-white px-3 py-2 text-sm text-brand-black outline-none transition focus:border-brand-gold focus:ring-2 focus:ring-brand-gold/20";
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded border border-brand-lea/20 px-2.5 py-1 text-xs font-semibold text-brand-lea transition hover:bg-brand-cloudDancer/60"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-lea/40 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-1 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-brand-lea">Add candidate</h2>
              <button onClick={() => setOpen(false)} className="rounded p-1 text-brand-grey hover:text-brand-lea" aria-label="Close"><X className="h-5 w-5" /></button>
            </div>
                    ...
                    results.map((c) => (
                      ...
                      >
                        <span className="min-w-0">
                          ...
                          <span className="block truncate text-xs text-brand-grey">{[c.currentTitle, c.primaryEmail].filter(Boolean).join(" · ") || "No details"}</span>
                        </span>
                        <Plus className="h-4 w-4 shrink-0 text-brand-eden" />
                      </button>
                    ))
                  ) : (
                    <p className="px-1 py-3 text-xs text-brand-grey">No matches. Try a different search, or add a new candidate.</p>
                  )}
                </div>
              </div>
            ) : (
