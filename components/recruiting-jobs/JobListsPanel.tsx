    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="break-words text-sm font-semibold text-brand-lea">{job.title}</div>
...
function CollapsibleList({
  ...
  return (
    <section
      className={`flex min-h-0 flex-col overflow-hidden rounded border border-brand-lea/10 bg-white transition-[flex] ${
    ...
    >
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        className="flex shrink-0 items-center justify-between gap-2 px-4 py-3 text-left transition hover:bg-brand-cloudDancer/45"
      >
        <span className="flex items-center gap-2">
          <Icon className="h-4 w-4 text-brand-gold" />
          <span className="text-base font-semibold text-brand-lea">{title}</span>
          <span className="text-xs text-brand-grey">({jobs.length})</span>
        </span>
        <ChevronDown
          className={`h-4 w-4 text-brand-grey transition-transform ${open ? "" : "-rotate-90"}`}
        />
      </button>
      {open ? (
        <div className="min-h-0 flex-1 overflow-y-auto border-t border-brand-lea/10 p-3">
          {jobs.length > 0 ? (
            ...
            <p className="p-2 text-sm text-brand-grey">{emptyText}</p>
          )}
        </div>
      ) : null}
    </section>
