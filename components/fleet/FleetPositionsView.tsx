...
function seatPill(seat: string) {
  return seat === "PIC" ? "bg-value-teamwork-light text-value-teamwork-dark" : "bg-brand-cloudDancer text-brand-grey";
}
    ...
    <div className="px-5 py-5 lg:px-8">
      <section className="mb-4 rounded bg-white p-5 shadow-panel ring-1 ring-brand-lea/10">
        ...
        <p className="mt-1 max-w-2xl text-sm text-brand-grey">
          ...
          Edit <span className="font-semibold text-brand-lea">FLEET_POSITIONS.md</span> and run{" "}
        ...
        {aircraftOrder.map((aircraft) => {
          ...
          return (
            <section
              key={aircraft}
              className={clsx(
                "rounded bg-white p-4 shadow-panel ring-1 ring-brand-lea/10",
            ...
            >
              ...
              <div className="mt-2 space-y-1.5">
                {positions.map((position) => (
                  <div
                    key={position.slug}
                    className="flex items-center justify-between gap-2 rounded border border-brand-lea/10 bg-brand-cloudDancer/30 px-3 py-2"
                  >
